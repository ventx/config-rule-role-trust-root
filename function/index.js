...
function evaluateChangeNotificationCompliance(configurationItem, ruleParameters) {
    ...
    if (configurationItem.resourceType !== 'AWS::IAM::Role') {
        ...
        const policyDocument = decodeURIComponent(configurationItem.configuration.assumeRolePolicyDocument)
        console.log(policyDocument)
        const policyObject = JSON.parse(policyDocument)
        if(policyObject.Statement.some((statement) => {
            console.log(statement.Principal)
          return JSON.stringify(statement.Principal).match("arn:aws:iam::[0-9]{12}:root")
        })) {
